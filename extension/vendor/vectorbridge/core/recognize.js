 ...
 *   diagonal length.
 */
export function recognizeShape(points, closedThreshold) {
    if (points.length < 2) {
...
// ─── Detectors ───────────────────────────────────────────────────────
function detectLine(points, bounds, diagonal) {
...
function triangleConfidence(corners, bounds) {
...
function rectangleConfidence(corners) {
...
function diamondConfidence(corners, bounds) {
...
function ellipseConfidence(points, bounds) {
    if (points.length < 8)
        return 0;
    ...
    const last = points[points.length - 1];
    const diagonal = Math.hypot(bounds.width, bounds.height);
    if (distanceBetween(first, last) > diagonal * 0.2)
        return 0;
...
function getInteriorAngles(corners) {
    const angles = [];
...
function removeDuplicateClosingPoint(points, threshold) {
    if (points.length < 2)
        return points;
    const first = points[0];
    const last = points[points.length - 1];
